...
if firebase_credentials:
    ...
    cred_path = 'forever-mavi-firebase-adminsdk.json'
...
def send_telegram_notification(order_number, cart_items, customer_details, final_total, payment_method=None, special_note=None):
    bot_token = "7678927978:AAGc3g4NIBbWKBGp9KrSrE6KhXVjjbEc05k"
    chat_id = "-1002825479493"
...
def join():
    if request.method == 'POST':
        try:
            ...
            message = f"New Join Request\nName: {name}\nPhone: {phone}\nEmail: {email}\nPackage: {package}\nTime: {time.strftime('%I:%M %p SAST, %B %d, %Y')}\nContact Mavi Dlamini to finalize! 📝"
            ...
            return jsonify({"message": "Join request submitted! Mavi will contact you soon. 🎉", "popup": True}), 200
        except Exception as e:
